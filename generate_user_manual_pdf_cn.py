...
with PdfPages(str(output_pdf_path)) as pdf:
    ...
    y -= 0.02
    p3_txt = (
        "横截面（垂直于传播方向，见图b）决定光限制与限制因子。有源区宽度 w_active 与厚度 d_active "
        "为完全参数化的输入，使得波导边界和模场腰部形状可以动态自适应调整。Elmer FEM在横向上求解耦合泊松方程（静电势）、"
    ...
    y -= 0.022
    m1_txt = (
        "提供全局耦合的多物理场扫参视图。左侧边栏包括设计滑块（反射率R1/R2、腔长L、有源区宽度w_active、"
        "有源区厚度d_active、冷却温度T0、注入电流）。右侧主显示面板包括六个同步的性能视口：纵向载流子N(z)分布曲线、"
        "纵向光功率P(z)曲线、2D横向波导模场形状、2D截面温度场图，以及波导横向/纵向中心切面的光强分布图。同时具有指示当前激射"
